...
import { getProfile, routeForStatus } from '@/lib/auth'

// Supabase exchanges the ?code in the URL for a session automatically
// (detectSessionInUrl). We just wait for the session, then route by status.
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    const finish = async () => {
      ...
      router.replace(profile ? routeForStatus(profile.status) : '/login')
    }
