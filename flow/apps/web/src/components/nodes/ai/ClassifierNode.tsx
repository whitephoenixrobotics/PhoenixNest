...
export function ClassifierNode({ id, data, selected }: Props) {
  const updateNodeConfig = useFlowStore((s) => s.updateNodeConfig)
  const output = useExecutionStore((s) => s.nodeStates[id]?.output) as
    | { score?: number; matched?: boolean; text?: string }
    | undefined
  const fileRef = useRef<HTMLInputElement>(null)

  const label    = (data.config?.label as string) ?? ''
  const examples = (data.config?.examples as string[]) ?? []
  const threshold = Number(data.config?.threshold ?? 0.75)
  ...
  // Grab a frame from any active webcam in the flow as a sample
  ...
  const canMatch = label.trim() !== '' && examples.length > 0

  return (
    <BaseNode id={id} data={data} selected={selected} icon={<BlockIcon type="classifier" size={16} className="text-violet-400" />}>
      <div className="w-[240px] space-y-2">
        {/* Label input */}
        <TextInput
          type="text"
          value={label}
          onChange={(e) => updateNodeConfig(id, { label: e.target.value })}
          placeholder="สิ่งที่ต้องการให้รู้จัก (เช่น แมว)"
        ...
        <div>
          <div className="text-[10px] text-zinc-500 mb-1">
            ตัวอย่าง {examples.length}/{MAX_EXAMPLES}
          </div>
          <div className="grid grid-cols-5 gap-1">
            {examples.map((src, i) => (
              <div key={i} className="relative aspect-square group overflow-hidden rounded border border-violet-500/50">
                ...
                />
                {/* X overlay — appears on hover, contained within the image */}
                <button
            ...
            {/* Add-image cell — only one, opens chooser between file/webcam */}
          ...
          {canMatch ? (
            output ? (
              <>
                ...
                {matched && (
                  <div className="text-center text-xs font-bold text-emerald-400 mt-1">
                    ✓ {label}
                  </div>
                )}
              </>
